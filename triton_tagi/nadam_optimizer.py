...
def _nadam_predict(m, S, mu_m, Sigma_m, C, Sigma_dmu, beta1, alpha_theta, alpha_m):
...
def _nadam_update(m, S, dm, dS, mu_m, Sigma_m, C, Sigma_dmu, S0, gamma, cap_factor, innov_clamp_c):
...
class NadamTAGI:
    ...
    def _update_param(self, layer, m_attr, S_attr, dm_attr, dS_attr, cap_factor):
    ...
    def step(self, x_batch, y_batch, sigma_v):
        ...
        for layer in self._learnable_list:
            self._update_param(layer, "mw", "Sw", "delta_mw", "delta_Sw", cap_factor)
            if getattr(layer, "has_bias", True):
                self._update_param(layer, "mb", "Sb", "delta_mb", "delta_Sb", cap_factor)
    ...
    def __repr__(self):
        ...
        return (
            f"NadamTAGI(β1={self.beta1}, γ={self.gamma}, "
