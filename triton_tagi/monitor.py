...
class TAGIMonitor:
    ...
    def __init__(
        self, net, log_dir="tagi_monitor", probe_size: int = 256, dead_thresh: float = 0.01
    ):
        self.net = net
    ...
    def _param_stats(self, idx, layer) -> ParamStats | None:
    ...
    def record(self, epoch: int, x_probe: torch.Tensor, tag: str = "") -> EpochRecord:
        """
        ...
        rec = EpochRecord(epoch=epoch, step=self._step, tag=tag, activations=acts, params=params)
    ...
    def print_report(self, record: EpochRecord | None = None):
        """
        ...
        print(f"\n  {'─' * 72}")
        print(f"  TAGI Monitor — epoch {record.epoch}  step {record.step}{tag_str}")
        print(f"  {'─' * 72}")
        ...
        for a in record.activations:
            ...
            print(
                f"  {a.layer_name:<22}  {a.mu_mean:>7.3f}  "
                f"{a.mu_std:>7.3f}  "
        ...
        if record.params:
            ...
            for p in record.params:
                ...
                if p.Sw_mean < 1e-10:
                    ...
                    sw_flag = " ⚠Sw↑↑"
                print(
                    f"  {p.layer_name:<22}  {p.mw_abs_mean:>9.5f}  "
                    f"{p.mw_std:>9.5f}  "
    ...
    def plot(self, filename: str | None = None, show: bool = False):
        """
        ...
        var_means = np.array(
            [[a.var_mean for a in r.activations] for r in self.history], dtype=np.float64
        )
        frac_dead = np.array(
            [[a.frac_dead for a in r.activations] for r in self.history], dtype=np.float64
        )
        ...
        if has_params:
            ...
            sw_hist = np.array(
                [[p.Sw_mean for p in r.params] for r in self.history], dtype=np.float64
            )

        # ── Layout ──
        fig = plt.figure(figsize=(18, 12))
        gs = gridspec.GridSpec(
            3, 2, figure=fig, hspace=0.48, wspace=0.32, left=0.07, right=0.97, top=0.93, bottom=0.06
        )
        ...
        im = ax1.imshow(log_var, aspect="auto", cmap=cmap_health, vmin=-4, vmax=2, origin="lower")
        ...
        ax1.set_title(
            "Activation Variance E[σ²]  per layer per epoch  (log₁₀ · green ≈ 1 · red = bad)",
            fontsize=9,
            fontweight="bold",
        )
        ...
        for _v, _lbl in [(-1, "0.1"), (0, "1.0"), (1, "10")]:
        ...
        colors = [
            "#d32f2f" if (v > 2 or v < -3) else "#f57c00" if (v > 1 or v < -2) else "#388e3c"
            for v in log_lv
        ]
        ax2.barh(range(n_layers), log_lv, color=colors, height=0.7)
        ...
        for threshold, color, style, lbl in [
            ...
            ax2.axvline(threshold, color=color, linestyle=style, linewidth=1, label=lbl, alpha=0.8)
        ax2.set_xlabel("log₁₀(E[σ²])", fontsize=8)
        ax2.set_title(f"Signal Flow — epoch {epochs[-1]}  (green=healthy, red=bad)", fontsize=8)
        ...
        for li in range(n_layers):
            ax3.semilogy(
                epochs,
                np.clip(var_means[:, li], 1e-10, None),
                label=act_names[li],
                alpha=0.8,
                color=cmap_layers(li),
                linewidth=1.2,
            )
        ax3.axhline(1.0, color="green", linestyle="--", linewidth=1, alpha=0.6, label="σ²=1 target")
        ax3.axhline(0.1, color="orange", linestyle=":", linewidth=1, alpha=0.5)
        ax3.axhline(100, color="red", linestyle="--", linewidth=1, alpha=0.5)
        ...
        if n_layers <= 12:
            ax3.legend(fontsize=5, loc="upper right", ncol=2, framealpha=0.5)
        ...
        if has_params:
            ...
            for pi in range(n_params):
                ax4.semilogy(
                    epochs,
                    np.clip(sw_hist[:, pi], 1e-15, None),
                    label=param_names[pi],
                    alpha=0.85,
                    color=cmap_params(pi),
                    linewidth=1.2,
                )
            ax4.axhline(
                1e-10, color="red", linestyle="--", linewidth=1, alpha=0.6, label="Sw→0 floor"
            )
            ax4.set_xlabel("Epoch", fontsize=8)
            ax4.set_ylabel("E[Sw]  (log scale)", fontsize=8)
            ax4.set_title("Weight Variance E[Sw]  (decreasing = model learning)", fontsize=8)
            ax4.tick_params(labelsize=7)
            if n_params <= 8:
                ax4.legend(fontsize=6, framealpha=0.5)
        else:
            ax4.text(
                0.5, 0.5, "No learnable layers", ha="center", va="center", transform=ax4.transAxes
            )
        ...
        ax5.axhline(50, color="red", linestyle="--", linewidth=1, alpha=0.6, label="50% dead")
        ax5.axhline(90, color="darkred", linestyle="--", linewidth=1, alpha=0.6, label="90% dead")
        ax5.set_ylim(0, 105)
        ...
        if n_layers <= 12:
            ax5.legend(fontsize=5, loc="upper right", ncol=2, framealpha=0.5)
...
def sweep_gains(
    ...
    ax.axhline(
        target_log, color="black", linestyle="--", linewidth=1.5, label=f"target σ²={target_var}"
    )
    ax.axhline(
        target_log + 2, color="red", linestyle=":", linewidth=1, alpha=0.6, label="×100 from target"
    )
    ax.axhline(
        target_log - 2,
        color="orange",
        ...
        linewidth=1,
        alpha=0.6,
        label="×0.01 from target",
    )
    ax.set_xticks(xs)
    ax.set_xticklabels(layer_names, rotation=45, ha="right", fontsize=7)
    ax.set_ylabel("log₁₀(E[σ²])", fontsize=9)
    ax.set_title(
        "Signal Variance per Layer at Init\n(want: all lines near the black dashed target)",
        fontsize=9,
    )
    ...
    ax2 = axes[1]
    heat = np.array([[math.log10(max(s.var_mean, 1e-10)) for s in results[g]] for g in gains])
    import matplotlib.colors as mcolors

    cmap_health = mcolors.LinearSegmentedColormap.from_list(
        "health", ["#d32f2f", "#f57c00", "#388e3c"], N=256
    )
    im = ax2.imshow(heat, aspect="auto", cmap=cmap_health, vmin=target_log - 3, vmax=target_log + 3)
    ax2.set_xticks(xs)
    ...
    ax2.set_title("Gain × Layer Heatmap  (green = σ² near target, red = bad)", fontsize=9)
    ...
    for gi in range(len(gains)):
        for li in range(n_layers):
            ax2.text(
                ...
                va="center",
                fontsize=5.5,
                color="white" if abs(heat[gi, li] - target_log) > 1.5 else "black",
            )

    fig.suptitle(
        f"Gain Sensitivity Analysis  (target σ²={target_var})", fontsize=12, fontweight="bold"
    )
    ...
    print(f"  {'─' * 60}")
    print(f"  {'Gain':>8}  {'Min log σ²':>11}  {'Max log σ²':>11}  {'Spread':>8}  Verdict")
    print(f"  {'─' * 60}")
    for gain in gains:
        log_vars = [math.log10(max(s.var_mean, 1e-10)) for s in results[gain]]
        lo, hi = min(log_vars), max(log_vars)
        spread = hi - lo
        if any(math.isnan(v) or math.isinf(v) for s in results[gain] for v in [s.var_mean]):
            verdict = "CRASH / NaN"
            ...
            verdict = "OK"
        print(f"  {gain:>8.4f}  {lo:>11.2f}  {hi:>11.2f}  {spread:>8.2f}  {verdict}")
    print(f"  {'─' * 60}")
...
def sweep_sigma_v(
    ...
    for sigma_v in sigma_vs:
        ...
        for i, layer in enumerate(net.layers):
            ...
            if layer.delta_mw is None:
                layer_stats.append(
                    dict(name="?", delta_mean=0.0, budget=0.0, frac_capped=0.0, ratio=0.0)
                )
                continue
    ...
    ax.loglog(sigma_vs, innov_means, "o-", color="steelblue", linewidth=2, label="E[|δμ|]")
    ax.loglog(sigma_vs, innov_maxs, "s--", color="tomato", linewidth=1.5, label="max|δμ|")
    # Add the cap budget (flat horizontal line per layer) as reference
    for _pi, (nm, bud) in enumerate(zip(param_names, Sw_budgets, strict=False)):
        ax.axhline(bud, linestyle=":", linewidth=1.0, alpha=0.5, label=f"cap {nm}")
    ...
    for si, sigma_v in enumerate(sigma_vs):
        frac_caps = [ls["frac_capped"] for ls in records[sigma_v]["layer_stats"]]
        ax2.plot(
            xs,
            [f * 100 for f in frac_caps],
            "o-",
            color=cmap(si),
            label=f"σ_v={sigma_v}",
            linewidth=1.5,
    ...
    ax2.set_ylim(-5, 105)
    ax2.set_title(
        "Fraction of Updates Hitting the Cap\n(100% = σ_v irrelevant, only Sw/gain matters)",
        fontsize=8,
    )
    ...
    ax3 = axes[2]
    for si, sigma_v in enumerate(sigma_vs):
        applied = [ls["applied"] for ls in records[sigma_v]["layer_stats"]]
        ax3.semilogy(
            xs,
            [max(a, 1e-15) for a in applied],
            "o-",
            color=cmap(si),
            label=f"σ_v={sigma_v}",
            linewidth=1.5,
    ...
    ax3.semilogy(
        xs,
        [max(b, 1e-15) for b in Sw_budgets],
        "k--",
        linewidth=2,
        label="cap budget √Sw/cap_f",
        alpha=0.7,
    )
    ax3.set_xticks(xs)
    ...
    ax3.set_title(
        "Actually Applied Update Magnitude\n"
    ...
    fig.suptitle(
        f"σ_v Sensitivity  (cap_factor={cap_factor}, batch={B})", fontsize=12, fontweight="bold"
    )
    ...
    print(f"  {'─' * 70}")
    print(f"  {'σ_v':>8}  {'E[|δμ|]':>10}  {'max|δμ|':>10}  {'avg %capped':>12}  Verdict")
    print(f"  {'─' * 70}")
    for sigma_v in sigma_vs:
        r = records[sigma_v]
        fc = [ls["frac_capped"] for ls in r["layer_stats"]]
        avg_cap = sum(fc) / len(fc) * 100 if fc else 0.0
        if avg_cap > 95:
        ...
        print(
            f"  {sigma_v:>8.4f}  {r['innov_mean']:>10.3e}  "
            f"{r['innov_max']:>10.3e}  {avg_cap:>11.1f}%  {verdict}"
        )
    print(f"  {'─' * 70}")
    print("  NOTE: cap budget (avg √Sw/cap_f):")
    for nm, bud, mw in zip(param_names, Sw_budgets, mw_magnitudes, strict=False):
        ratio = bud / max(mw, 1e-12) * 100
        print(f"    {nm:<24} budget={bud:.3e}  |mw|={mw:.3e}  budget/|mw|={ratio:.2f}%")
...
def compare_heads(
    ...
    ax.hist(
        mp_rm, bins=30, alpha=0.6, color=color_rm, label=f"Remax  μ={max_P_rm:.3f}  H={H_rm:.2f}"
    )
    ax.hist(
        mp_bn,
        bins=30,
    ...
    ax.set_title(
        "Output Variance V = Bernoulli/Remax variance\n"
        "(large V → σ_v has less relative influence on δμ)",
        fontsize=9,
    )
    ax.legend(fontsize=7)
    ...
    ax = axes[1, 0]
    ax.hist(j_rm_f, **kw, color=color_rm, label=f"Remax  E[|J|]={J_abs_rm:.4f}")
    ax.hist(j_bn_f, **kw, color=color_bn, label=f"Bernoulli  E[|J|]={J_abs_bn:.4f}")
    ax.set_xlabel("J_k = cov(z_k, a_k) / Sz_k", fontsize=8)
    ax.set_title(
        "Backward Jacobian J\n(larger |J| = stronger gradient reaching upstream layers)", fontsize=9
    )
    ax.legend(fontsize=7)
    ...
    ax.scatter(
        np.log10(np.clip(sz_f[idx], 1e-10, None)),
        j_rm_f[idx],
        s=3,
        alpha=0.4,
        color=color_rm,
    ...
    rows = [
        ...
        ("J std  (spread)", f"{J_rm.std().item():.4f}", f"{J_bn.std().item():.4f}"),
    ]
    for label, rm, bn in rows:
        print(f"  {label:<30}  {rm:>10}  {bn:>10}")
    print(f"  {'─' * 62}")
    print("  σ_v crossover (E[V]=σ_v²):")
    print(f"    Remax    σ_v* ≈ {math.sqrt(s_pred_rm):.4f}")
    print(f"    Bernoulli σ_v* ≈ {math.sqrt(s_pred_bn):.4f}")
    print("  For σ_v >> σ_v*: head variance dominates → innovation ≈ 1/S_pred")
    print("  For σ_v << σ_v*: observation precision dominates → innov. ≈ 1/σ_v²")
    print(f"  {'─' * 62}")
