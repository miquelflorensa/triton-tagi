...
def he_init(fan_in: float) -> float:
    """He initialization scale (He et al., 2015).

    For ReLU activations, the variance-preserving condition requires
    S_W = 2 / fan_in.  Since gaussian_param_init computes
    S = (gain * scale)^2, we need scale = sqrt(2 / fan_in) so that
    S_W = scale^2 = 2 / fan_in  (with gain = 1.0).
...
def gaussian_param_init(scale: float, gain: float, shape, device="cuda"):
    ...
    S = torch.full(shape if isinstance(shape, tuple) else (shape,),
                   (gain * scale) ** 2, device=device)
    return m, S
...
def init_weight_bias_linear(input_size: int, output_size: int,
                            init_method: str = "He",
                            gain_w: float = 1.0, gain_b: float = 1.0,
                            bias: bool = True, device="cuda"):
    ...
    mu_w, var_w = gaussian_param_init(scale, gain_w,
                                      (input_size, output_size), device)

    if bias:
        mu_b, var_b = gaussian_param_init(scale, gain_b,
                                          (1, output_size), device)
    else:
...
def init_weight_bias_conv2d(kernel_size: int, in_channels: int,
                            out_channels: int,
                            init_method: str = "He",
                            gain_w: float = 1.0, gain_b: float = 1.0,
                            device="cuda"):
    ...
    mu_w, var_w = gaussian_param_init(scale, gain_w,
                                      (K, out_channels), device)
    mu_b, var_b = gaussian_param_init(scale, gain_b,
                                      (1, out_channels), device)

    return mu_w, var_w, mu_b, var_b


def init_weight_bias_norm(num_features: int,
                          gain_w: float = 1.0, gain_b: float = 1.0,
                          device="cuda"):
