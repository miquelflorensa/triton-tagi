...
class SharedVarResBlock:
    ...
    def __init__(self, in_channels, out_channels, stride=1,
        ...
        # ── Main path ──
        self.conv1 = SharedVarConv2D(in_channels, out_channels, kernel_size=3,
                                     stride=stride, padding=1, device=device,
                                     gain_w=gain_w, gain_b=gain_b)
        self.relu1 = ReLU()
        self.bn1 = SharedVarBatchNorm2D(out_channels, device=device,
                                        gain_w=gain_w, gain_b=gain_b,
                                        preserve_var=False)

        self.conv2 = SharedVarConv2D(out_channels, out_channels, kernel_size=3,
                                     stride=1, padding=1, device=device,
                                     gain_w=gain_w, gain_b=gain_b)
        self.relu2 = ReLU()
        self.bn2 = SharedVarBatchNorm2D(out_channels, device=device,
                                        gain_w=gain_w, gain_b=gain_b,
                                        preserve_var=False)
        ...
        if self.use_projection:
            self.proj_conv = SharedVarConv2D(in_channels, out_channels,
                                            kernel_size=2, stride=stride,
                                            padding=0, device=device,
                                            gain_w=gain_w, gain_b=gain_b)
            self.proj_relu = ReLU()
            self.proj_bn = SharedVarBatchNorm2D(out_channels, device=device,
                                                gain_w=gain_w, gain_b=gain_b,
                                                preserve_var=False)
    ...
    def __repr__(self):
        ...
        return (f"SharedVarResBlock({self.in_channels}→{self.out_channels}, "
