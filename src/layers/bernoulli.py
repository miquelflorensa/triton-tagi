...
@triton.jit
def _hermite_kernel(
    mu_ptr, sig_ptr,            # (B, K)  contiguous
    nodes_ptr, weights_ptr,     # (n_gh,)
    P_ptr, EXf_ptr,             # (B, K)  output
...
def triton_bernoulli_moments(mu_z, sigma_z_sq, n_gh=32):
    ...
    _hermite_kernel[(B * K,)](
        mu_z, sigma_z, nodes, weights, P, EXf,
        K, n_gh, mu_z.stride(0),
        N_GH=N_GH,
    )
...
class Bernoulli:
    ...
    def __init__(self, n_gh=32, top_m=None):
        self.n_gh  = n_gh
        self.top_m = top_m
    ...
    def forward(self, mz, Sz):
        ...
        if self.top_m is not None:
            P, V, C = triton_bernoulli_moments_fast(
                mz, Sz, n_gh=self.n_gh, top_m=self.top_m
            )
        else:
