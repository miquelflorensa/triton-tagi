...
from .monitor import TAGIMonitor, ActivationStats
...
def _score_gain(stats: List[ActivationStats], target_var: float,
                gain_w: float, gain_b: float) -> GainScore:
    """
    ...
    if any(math.isnan(s.var_mean) or math.isinf(s.var_mean) for s in stats):
        return GainScore(
            gain_w=gain_w, gain_b=gain_b, total_score=-1000.0,
            centering=-100.0, spread=100.0,
            dead_penalty=0.0, explode_penalty=0.0,
            nan_crash=True, verdict="CRASH",
            per_layer_log_var=[],
        )
    ...
    return GainScore(
        gain_w=gain_w, gain_b=gain_b, total_score=total,
        centering=centering, spread=spread,
        dead_penalty=dead_penalty, explode_penalty=explode_penalty,
        nan_crash=False, verdict=verdict,
...
def find_best_gain(
    ...
    gains_w: Optional[List[float]] = None,
    bias_factors: Optional[List[float]] = None,
    ...
    if verbose:
        ...
        print(f"\n  Phase 1: Gain Search ({n_total} candidates, "
    ...
    for gw in gains_w:
        for bf in bias_factors:
            ...
            try:
                ...
                score = GainScore(
                    gain_w=gw, gain_b=gb, total_score=-1000.0,
                    centering=-100.0, spread=100.0,
                    dead_penalty=0.0, explode_penalty=0.0,
                    nan_crash=True, verdict="CRASH",
                    per_layer_log_var=[],
    ...
    if refine and not best.nan_crash:
        ...
        if verbose:
            print(f"\n  Refining around gain_w={best_gw:.4f} "
                  f"(±0.5 decades, {n_refine} points)")
        ...
        if verbose:
            print(f"  Refined best: gain_w={best.gain_w:.4f}, "
...
def _print_gain_table(scores: List[GainScore], target_var: float):
    ...
    print(f"\n  {'gain_w':>8}  {'gain_b':>8}  {'Score':>7}  "
          f"{'Center':>7}  {'Spread':>7}  Verdict")
    print(f"  {'─'*60}")
    for s in scores:
        print(f"  {s.gain_w:>8.4f}  {s.gain_b:>8.4f}  {s.total_score:>7.2f}  "
...
def _score_sigma_v(accuracy_after, accuracy_delta, avg_frac_capped,
                   avg_param_movement, sigma_v) -> SigmaVScore:
    """
...
def find_best_sigma_v(
    ...
    sigma_vs: Optional[List[float]] = None,
    n_steps: int = 50,
    batch_size: int = 128,
    ...
    if verbose:
        print(f"\n  Phase 2: sigma_v Search (n_steps={n_steps}, "
    ...
    for sv in sigma_vs:
        ...
        if verbose:
            print(f"  σ_v={sv:.3e}  acc={acc_after*100:5.1f}%  "
...
def auto_tune(
    ...
    probe_size: int = 256,
    verbose: bool = True,
    plot_filename: Optional[str] = "auto_tune.png",
) -> TuneResult:
    """
...
def _print_summary(result: TuneResult):
    ...
    print(f"\n  {'='*62}")
    print(f"  TAGI Auto-Tune Results")
    print(f"  {'='*62}")
    ...
    print(f"    Score: {bs.total_score:.2f}  "
          f"(centering={bs.centering:.2f}, spread={bs.spread:.2f})")
    ...
    print(f"    Accuracy: {ss.accuracy_after*100:.1f}%  "
          f"(delta={ss.accuracy_delta*100:+.1f}%)")
...
def _plot_tune_results(result: TuneResult, filename: str, verbose: bool = True):
    ...
    if gw_best:
        ...
        ax.scatter([math.log10(g) for g in gws], scores_y, c=colors,
                   s=60, zorder=5, edgecolors="black", linewidths=0.5)
        ax.plot([math.log10(g) for g in gws], scores_y, "k-", alpha=0.3)
        ax.axvline(math.log10(gr.best_gain_w), color="green",
                   linestyle="--", linewidth=2, alpha=0.7, label="best")

    ax.set_xlabel("log10(gain_w)", fontsize=9)
    ax.set_ylabel("Score", fontsize=9)
    ax.set_title("Gain Score Landscape\n"
                 "(green=GOOD, blue=OK, orange=VANISHING, red=EXPLODING)",
                 fontsize=9)
    ...
    if best_gs.per_layer_log_var:
        ...
        ax2.axhline(target_log, color="black", linestyle="--",
                    linewidth=1.5, label=f"target={gr.target_var}")
        ax2.axhline(target_log + 2, color="red", linestyle=":",
                    linewidth=1, alpha=0.5)
        ax2.axhline(target_log - 2, color="orange", linestyle=":",
                    linewidth=1, alpha=0.5)

    ax2.set_xlabel("Layer index", fontsize=9)
    ax2.set_ylabel("log10(E[var])", fontsize=9)
    ax2.set_title(f"Per-Layer Variance at Best Gain\n"
    ...
    ax3.plot([math.log10(sv) for sv in sv_vals], sv_scores,
             "o-", color="steelblue", linewidth=2, markersize=6)
    ax3.axvline(math.log10(sr.crossover_sigma_v), color="gray",
                linestyle=":", linewidth=1.5, alpha=0.7,
                label=f"crossover σ_v*={sr.crossover_sigma_v:.2e}")
    ax3.axvline(math.log10(sr.best_sigma_v), color="green",
    ...
    ax4_acc = ax4
    ax4_acc.plot(svs_log, [s.accuracy_delta * 100 for s in sr.all_scores],
                 "o-", color="#1976D2", linewidth=2, label="Δacc (%)")
    ax4_acc.set_ylabel("Δ accuracy (%)", fontsize=9, color="#1976D2")
    ax4_acc.tick_params(axis="y", labelcolor="#1976D2")

    ax4b = ax4.twinx()
    ax4b.plot(svs_log, [s.avg_frac_capped * 100 for s in sr.all_scores],
              "s--", color="#d32f2f", linewidth=1.5, alpha=0.8, label="% capped")
    ...
    ax4.set_title("Training Dynamics vs sigma_v\n"
