...
def test_add_layer():
    ...
    from triton_tagi.layers.resblock import Add
...
def test_identity_forward():
    ...
    from triton_tagi.layers.resblock import ResBlock
...
def test_architecture_details():
    ...
    from triton_tagi.layers.batchnorm2d import BatchNorm2D
    from triton_tagi.layers.conv2d import Conv2D
    from triton_tagi.layers.relu import ReLU
    from triton_tagi.layers.resblock import ResBlock

    # Identity block
    block = ResBlock(64, 64, stride=1, device=DEVICE)
    n = len(block._main_layers)
    assert n == 6, f"Main path should have 6 layers, got {n}"
    ...
    n = len(block._proj_layers)
    assert n == 3, f"Proj path should have 3 layers, got {n}"
