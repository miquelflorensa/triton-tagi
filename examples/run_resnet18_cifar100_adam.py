"""
CIFAR-100 Classification with Adam-TAGI ResNet-18
==================================================
Same ResNet-18 as run_resnet18_cifar100.py, but uses AdamTAGI instead of
the standard capped Bayesian update.

Comparison:
    python run_resnet18_cifar100.py          # standard TAGI (baseline)
    python run_resnet18_cifar100_adam.py      # Adam-TAGI (this script)
"""
...
# ======================================================================
#  Checkpointing (saves optimizer EMA state too)
# ======================================================================
...
def load_checkpoint(net, opt, path):
    ...
    if path == "latest":
        candidates = sorted(_glob.glob(
            os.path.join("run_logs_adam_cifar100", "checkpoints",
...
def train(opt, x_train, y_train_oh, y_train_labels, x_test, y_test_labels,
          ...
          label_smooth=0.0,
          checkpoint_dir="run_logs_adam_cifar100/checkpoints",
          start_epoch=1):
...
def main():
    parser = argparse.ArgumentParser(
        description="Train TAGI ResNet-18 on CIFAR-100 with Adam-TAGI")
    ...
    parser.add_argument("--eps-Q", type=float, default=1e-8)
    args = parser.parse_args()

    print("=" * 62)
    print("  CIFAR-100 — Adam-TAGI ResNet-18")
    print("  Same architecture as run_resnet18_cifar100.py + Adam-TAGI")
    print("=" * 62)
    ...
    # ── Monitor ──
    monitor = TAGIMonitor(net, log_dir="run_logs_adam_cifar100", probe_size=256)
    net.eval()
    ...
    # ── Save logs ──
    monitor.plot("run_logs_adam_cifar100/resnet18_adam_monitor.png")
    monitor.save_csv("run_logs_adam_cifar100/resnet18_adam_monitor.csv")
