"""
CIFAR-10 Classification with Nadam-TAGI Optimizer
==================================================
Same 3-block CNN as run_cifar10.py, but uses NadamTAGI instead of the
standard capped Bayesian update.

Comparison:
    python run_cifar10.py           # standard TAGI (baseline)
    python run_cifar10_adam.py      # Adam-TAGI
    python run_cifar10_nadam.py     # Nadam-TAGI (this script)
"""
...
def train(opt, x_train, y_train_oh, y_train_labels, x_test, y_test_labels,
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for i in range(0, len(x_s), batch_size):
            ...
            # ── Nadam-TAGI step (replaces net.step) ──
...
def main():
    print("=" * 60)
    print("  CIFAR-10 — Nadam-TAGI Optimizer Experiment")
    print("  Same 3-block CNN, swapping capped update for Nadam-TAGI")
    print("=" * 60)
    ...
    gamma       = 0.99      # process noise EMA smoothing
    alpha_theta = 1e-3      # process noise scaling (Eq. 10)
    alpha_m     = 1e-3      # process noise scaling (Eq. 10)
    sigma_m0_sq = 1e-8      # initial velocity variance (Eq. 3)
    eps_Q       = 1e-6      # initial Σ_{Δμ} seed (§2.3.4)

    opt = NadamTAGI(net, beta1=beta1, gamma=gamma,
                    alpha_theta=alpha_theta, alpha_m=alpha_m,
                    sigma_m0_sq=sigma_m0_sq, eps_Q=eps_Q)
    print(f"\n  Optimizer: {opt}")

    # ── Monitor ──
    monitor = TAGIMonitor(net, log_dir="run_logs_nadam", probe_size=256)
    net.eval()
    ...
    print(f"\n  Nadam-TAGI config:")
    print(f"    β1={beta1}, γ={gamma}, α_θ={alpha_theta}, α_m={alpha_m}")
    print(f"    σ²_m0={sigma_m0_sq}, ε_Q={eps_Q}")
    ...
    # ── Save logs ──
    monitor.plot("run_logs_nadam/monitor.png")
    monitor.save_csv("run_logs_nadam/monitor.csv")
