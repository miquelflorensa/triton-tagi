"""
CIFAR-100 Classification with Shared-Variance TAGI ResNet-18
==============================================================
Same architecture as run_resnet18_cifar100.py but uses shared-variance layers:
...
Architecture:
  ...
  Head:     AvgPool(4) → Flatten → SharedVarLinear(512→100) → Remax
"""
...
# ======================================================================
#  Depth-scaled gain function (same as run_resnet18_cifar100.py)
# ======================================================================
...
# ======================================================================
#  Network builder — Shared-Variance ResNet-18 for CIFAR-100
# ======================================================================

def build_resnet18_shared_var(num_classes=NUM_CLASSES, head="remax",
                              device="cuda", gain_w=1.0, gain_b=1.0,
                              g_min=0.1, g_max=0.5):
    """
    Build a Shared-Variance ResNet-18 for CIFAR-100.

    Same architecture as run_resnet18_cifar100.py but using SharedVar* layers.
    """
    N_SLOTS = 11
...
def save_checkpoint(net, epoch, save_dir):
    ...
    def _layer_state(layer):
        ...
        elif hasattr(layer, "Sw") and isinstance(layer.Sw, torch.Tensor):
            d["Sw"] = layer.Sw.cpu().clone()
            d["Sb"] = layer.Sb.cpu().clone()
...
def load_checkpoint(net, path: str) -> int:
    ...
    if path == "latest":
        candidates = sorted(_glob.glob(
            os.path.join("run_logs_shared_var_cifar100", "checkpoints",
                         "checkpoint_epoch_*.pt")))
        if not candidates:
            raise FileNotFoundError(
                "No checkpoints found in run_logs_shared_var_cifar100/checkpoints/")
        path = candidates[-1]
    ...
    def _restore(layer, d):
        ...
        if "sw" in d:
            layer.sw.copy_(d["sw"].to(DEVICE))
            layer.sb.copy_(d["sb"].to(DEVICE))
        elif "Sw" in d:
...
def train(net, x_train, y_train_oh, y_train_labels, x_test, y_test_labels,
          ...
          label_smooth=0.0,
          checkpoint_dir="run_logs_shared_var_cifar100/checkpoints",
          start_epoch=1):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train Shared-Variance TAGI ResNet-18 on CIFAR-100")
    ...
    print("=" * 66)
    print("  CIFAR-100 — Shared-Variance TAGI ResNet-18")
    ...
    print("  Head:     AvgPool(4) → Flatten → SharedVarFC(512→100) → Remax")
    print("=" * 66)
    ...
    # Match run_resnet18_cifar100.py: eta=0.125 for 100 classes
    eta = 0.125
    ...
    # ── Monitor ──
    monitor = TAGIMonitor(net, log_dir="run_logs_shared_var_cifar100",
                          probe_size=256)
    net.eval()
    ...
    # ── Hyperparameters (matching run_resnet18_cifar100.py) ──
    batch_size      = 128
    n_epochs        = 100
    monitor_every   = 5
    label_smooth    = 0.0
    checkpoint_dir  = "run_logs_shared_var_cifar100/checkpoints"
    # CIFAR-100 baseline uses decay_factor=1.0 (constant σ_v)
    decay_factor    = 1.0
    ...
    # ── Save logs ──
    monitor.plot("run_logs_shared_var_cifar100/resnet18_sv_monitor.png")
    monitor.save_csv("run_logs_shared_var_cifar100/resnet18_sv_monitor.csv")
