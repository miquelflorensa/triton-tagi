"""
Quick comparison: cap-factor vs precision-space parameter update on MNIST MLP.

Runs depth=5 MLP (784->256->256->256->256->256->10) for 20 epochs under
three update modes and prints a side-by-side accuracy table.
"""
...
def load_mnist(data_dir="data", device=torch.device("cpu"), C=1.0):
    ...
    oh_te = torch.full((len(y_te), 10), -C, device=device)
    oh_te.scatter_(1, y_te.unsqueeze(1), C)

    return x_train, oh_tr, x_test, y_te, oh_te
...
def build_net(depth, hidden, device, gain=1.0, update_mode="cap", rho_mode="full", rho=1.0):
    layers = []
    in_f = 784
    for _ in range(depth):
        ...
        layers.append(ReLU())
        in_f = hidden
    layers.append(Linear(in_f, 10, device=device, gain_w=gain, gain_b=gain))
    return Sequential(layers, device=device, update_mode=update_mode, rho_mode=rho_mode, rho=rho)


def run_one(label, update_mode, rho_mode, rho, depth, hidden, gain, sigma_v,
            n_epochs, batch_size, x_train, y_train, x_test, y_labels, device, seed):
    torch.manual_seed(seed)
    net = build_net(depth, hidden, device, gain=gain,
...
def main(
    ...
    x_train, y_train, x_test, y_labels, _ = load_mnist(data_dir, dev, C=target_scale)
    ...
    print()
    print(f"  depth={depth}  hidden={hidden}  gain={gain}  sigma_v={sigma_v}")
    print(f"  batch={batch_size}  epochs={n_epochs}  C=±{target_scale}")
    print()
    ...
    for label, um, rm, r in configs:
        best, final, wall = run_one(
            label, um, rm, r,
            depth, hidden, gain, sigma_v,
            n_epochs, batch_size,
            x_train, y_train, x_test, y_labels,
            dev, seed,
        )
