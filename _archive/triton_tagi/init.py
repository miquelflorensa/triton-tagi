...
from .param_init import init_weight_bias_conv2d, init_weight_bias_linear, init_weight_bias_norm
...
def reinit_net(net, verbose=True):
    ...
    for i, layer in enumerate(net.layers):
        if isinstance(layer, Linear):
            ...
            if verbose:
                print(
                    f"  Layer {i:2d} (Linear  ): in={layer.in_features}, out={layer.out_features}"
                )
            ...
            if verbose:
                print(
                    f"  Layer {i:2d} (ResBlock): {layer.in_channels}→{layer.out_channels},"
    ...
    for i, layer in enumerate(net.layers):
        if isinstance(layer, (ResBlock, SharedVarResBlock)):
            ...
            if verbose:
                ...
                if layer.use_projection:
                    proj_str = "  [projection]"
                print(f"  Layer {i:2d} (ResBlock): bn2.γ = {gamma:.4f}{proj_str}")

        elif isinstance(layer, FRNResBlock):
            # Set second FRN's gamma to √η (same idea, attenuate branch)
            layer.frn2.mw = torch.full_like(layer.frn2.mw, gamma)

            if verbose:
                proj_str = ""
                if layer.use_projection:
                    proj_str = "  [projection]"
                print(f"  Layer {i:2d} (FRNResBlock): frn2.γ = {gamma:.4f}{proj_str}")

    if verbose:
        n_blocks = sum(
            1 for lay in net.layers if isinstance(lay, (ResBlock, SharedVarResBlock, FRNResBlock))
        )
        expected_var = (1.0 + eta) ** n_blocks
        print("  ──────────────────────────────────────────")
        print(f"  Expected σ² after {n_blocks} blocks: (1+{eta})^{n_blocks} = {expected_var:.3f}")
        print("=" * 60)
