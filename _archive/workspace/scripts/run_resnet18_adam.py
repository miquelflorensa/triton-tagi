"""
CIFAR-10 Classification with Adam-TAGI ResNet-18
=================================================
Same ResNet-18 as run_resnet18.py, but uses AdamTAGI instead of the
standard capped Bayesian update.

Comparison:
    python run_resnet18.py          # standard TAGI (baseline)
    python run_resnet18_adam.py      # Adam-TAGI (this script)
"""
...
from triton_tagi.layers import (Linear, ReLU, Conv2D, Flatten, Remax, Bernoulli,
                        BatchNorm2D, AvgPool2D, ResBlock)
from triton_tagi.optimizer import AdamTAGI
...
# ======================================================================
#  Depth-scaled gain (from run_resnet18.py)
# ======================================================================
...
# ======================================================================
#  Network builder — ResNet-18 (from run_resnet18.py)
# ======================================================================
...
def load_checkpoint(net, opt, path):
    ...
    if path == "latest":
        candidates = sorted(_glob.glob(
            os.path.join("run_logs_adam_resnet", "checkpoints",
    ...
    if "opt_state" in ck:
        ...
        # EMA states are lazily re-created, so we pre-populate them
        # by triggering _get_state for each layer, then overwriting
        for layer in opt._learnable_list:
            for attr in ('mw', 'mb'):
                if attr == 'mb' and not getattr(layer, 'has_bias', True):
                    continue
                opt._get_state(layer, attr)
        # Now overwrite with saved values (keyed by string repr of tuple)
...
def train(opt, x_train, y_train_oh, y_train_labels, x_test, y_test_labels,
          ...
          decay_factor=0.90, min_sigma_v=0.01,
          label_smooth=0.0,
          checkpoint_dir="run_logs_adam_resnet/checkpoints",
          start_epoch=1):
...
def main():
    parser = argparse.ArgumentParser(
        description="Train TAGI ResNet-18 on CIFAR-10 with Adam-TAGI")
    ...
    print("=" * 62)
    print("  CIFAR-10 — Adam-TAGI ResNet-18")
    print("  Same architecture as run_resnet18.py + Adam-TAGI optimizer")
    print("=" * 62)
    ...
    # ── Monitor ──
    monitor = TAGIMonitor(net, log_dir="run_logs_adam_resnet", probe_size=256)
    net.eval()
    ...
    # ── Save logs ──
    monitor.plot("run_logs_adam_resnet/resnet18_adam_monitor.png")
    monitor.save_csv("run_logs_adam_resnet/resnet18_adam_monitor.csv")
