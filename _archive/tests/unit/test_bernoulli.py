...
from triton_tagi.layers.bernoulli import (
    Bernoulli,
...
requires_cuda = pytest.mark.skipif(not CUDA_AVAILABLE, reason="Triton kernel requires CUDA")
...
def make_inputs(B, K, mu_scale=1.0, var_lo=0.1, var_hi=2.0, device=DEVICE, seed=0):
    """Return (mu_z, sigma_z_sq) with reasonable random values."""
    g = torch.Generator(device=device)
    g.manual_seed(seed)
    mu_z = torch.randn(B, K, device=device, generator=g) * mu_scale
    sigma_z_sq = torch.rand(B, K, device=device, generator=g) * (var_hi - var_lo) + var_lo
...
class TestMathProperties:
    ...
    def test_probs_sum_to_one_various_k(self):
        ...
        for K in [2, 5, 10, 50]:
            ...
            sums = P.sum(dim=-1)
            assert torch.allclose(sums, torch.ones_like(sums), atol=1e-4), f"K={K}: sum(P)={sums}"
    ...
    def test_variance_equals_bernoulli_formula(self):
        ...
        diff = (V - expected).abs()
        assert diff.max() < 1e-5, f"Variance formula mismatch: max diff={diff.max():.2e}"
    ...
    def test_k1_trivial(self):
        ...
        P, V, C = triton_bernoulli_moments(mu_z, Sz)
        assert torch.allclose(P, torch.ones_like(P), atol=1e-4), f"K=1: P={P} (expected all 1s)"
    ...
    def test_k2_symmetry(self):
        ...
        assert torch.allclose(P, torch.full_like(P, 0.5), atol=1e-4), f"K=2 symmetric: P={P}"
    ...
    def test_uniform_means_give_uniform_probs(self):
        ...
        for K in [2, 5, 10]:
            ...
            assert diff < 1e-3, f"K={K} uniform: max |P - 1/K| = {diff:.3e}"
    ...
    def test_dominant_class_wins(self):
        ...
        P, V, C = triton_bernoulli_moments(mu_z, Sz)
        assert (P[:, 2] > 0.999).all(), f"Dominant class P={P[:, 2]} (expected > 0.999)"
    ...
    def test_higher_mean_higher_prob(self):
        ...
        Sz = torch.ones(1, 2, device=DEVICE)
        P, V, C = triton_bernoulli_moments(mu_z, Sz)
        assert P[0, 0] > P[0, 1], f"Expected P[0]>P[1] but got {P[0, 0]:.4f} vs {P[0, 1]:.4f}"
...
class TestMonteCarlo:
    ...
    def test_k2_vs_mc(self):
        ...
        assert err.max() < 0.02, f"K=2 MC error={err.max():.4f}\n  GH: {P}\n  MC: {P_mc}"
    ...
    def test_k5_vs_mc(self):
        ...
        P_mc = mc_bernoulli(mu_z, Sz)
        err = (P - P_mc.to(DEVICE)).abs()
        assert err.max() < 0.02, f"K=5 MC error={err.max():.4f}\n  GH: {P}\n  MC: {P_mc}"
    ...
    def test_k10_vs_mc(self):
        ...
        P_mc = mc_bernoulli(mu_z, Sz)
        err = (P - P_mc.to(DEVICE)).abs()
        assert err.max() < 0.03, f"K=10 MC error={err.max():.4f}\n  GH: {P}\n  MC: {P_mc}"
    ...
    def test_batch_k5_vs_mc(self):
        ...
        P_mc = mc_bernoulli(mu_z, Sz)
        err = (P - P_mc.to(DEVICE)).abs()
        assert err.max() < 0.03, f"Batched MC max error={err.max():.4f}"
    ...
    def test_covariance_vs_mc(self):
        ...
        assert err.max() < 0.03, f"Covariance MC error={err.max():.4f}\n  GH: {C}\n  MC: {C_mc}"
...
class TestNumericalStability:
    ...
    def test_large_variance_approaches_uniform(self):
        ...
        P, _, _ = triton_bernoulli_moments(mu_z, Sz, n_gh=32)
        assert (P - 1.0 / K).abs().max() < 0.05, f"Large-variance equal-means: P = {P}"
    ...
    def test_mixed_variance_no_crash(self):
        ...
        Sz = torch.tensor([[1e-8, 0.5, 1.0, 5.0, 1e4], [1e-8, 0.5, 1.0, 5.0, 1e4]], device=DEVICE)
    ...
    def test_uniform_large_k_symmetry(self):
        ...
        assert dev < 0.01, f"K={K} uniform: symmetry deviation too large: {dev:.3e}"
...
class TestTopM:
    ...
    def test_top_m_equals_k_matches_full(self):
        ...
        P_fast, V_fast, C_fast = triton_bernoulli_moments_fast(mu_z, Sz, n_gh=32, top_m=10)
    ...
    def test_top_m_clear_winner_accuracy(self):
        ...
        assert err.max() < 0.01, f"top_m=3 vs full with clear winner: max|ΔP|={err.max():.4f}"
    ...
    def test_top_m_1_winner_take_all(self):
        ...
        for b in range(4):
            assert P[b, winner[b]] > 0.99, f"Batch {b}: winner P={P[b, winner[b]]:.4f}"
            others = torch.cat([P[b, : winner[b]], P[b, winner[b] + 1 :]])
            assert (others < 1e-4).all(), f"Batch {b}: non-winner classes not zero: {others}"
    ...
    def test_top_m_equal_means_diagnostic(self):
        ...
        for top_m in [3, 5, 10, 15]:
            P_fast, _, _ = triton_bernoulli_moments_fast(mu_z, Sz, top_m=top_m, n_gh=32)
    ...
    def test_top_m_0_edge_case(self):
        ...
        try:
            ...
            assert (sums < 0.01).all(), "top_m=0 unexpectedly gave valid probabilities (bug fixed?)"
        except Exception as e:
    ...
    def test_fast_v_clamp_inconsistency(self):
        ...
        print(
            f"  Full path clamps V to 1e-7 ({v_min_full:.2e}), "
...
class TestLayerAPI:
    ...
    def test_backward_output_shapes(self):
        ...
        d_mz, d_Sz = layer.backward(
            torch.ones(B, K, device=DEVICE), torch.ones(B, K, device=DEVICE)
        )
    ...
    def test_backward_delta_sz_non_negative(self):
        ...
        layer = Bernoulli(n_gh=16)
        layer.forward(mu_z, Sz)
        _, d_Sz = layer.backward(torch.zeros(B, K, device=DEVICE), torch.ones(B, K, device=DEVICE))
        assert (d_Sz >= -1e-7).all(), f"Negative delta_Sz: min={d_Sz.min():.3e}"
    ...
    def test_cauchy_schwarz_holds(self):
        ...
        assert excess.max() < 1e-5, f"Cauchy-Schwarz violated: max excess={excess.max():.3e}"
    ...
    def test_jacobian_near_zero_variance(self):
        ...
        layer.forward(mu_z, Sz)
        J = layer.J
        print(f"\n  Tiny Sz: J.abs().max()={J.abs().max():.3e}, J.abs().min()={J.abs().min():.3e}")
...
class TestShapes:
    ...
    def test_1d_vs_2d_consistency(self):
        ...
        P2, V2, C2 = triton_bernoulli_moments(mu_z.unsqueeze(0), Sz.unsqueeze(0))
    ...
    def test_k2_b1_directional(self):
        ...
        P, V, C = triton_bernoulli_moments(mu_z, Sz)
        assert P.shape == (1, 2)
        assert P[0, 0] > P[0, 1], f"Higher mean should yield higher P: {P}"
    ...
    def test_fast_1d_vs_2d(self):
        ...
        P2, V2, C2 = triton_bernoulli_moments_fast(mu_z.unsqueeze(0), Sz.unsqueeze(0), top_m=4)
        assert torch.allclose(P1, P2.squeeze(0), atol=1e-6)
