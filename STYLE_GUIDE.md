...
| `yields` / `yielding` | Overused LLM verb. I say "produces," "gives," or "results in" instead. |
...
- **Possessive 's** is reserved for persons and places (e.g., "Bayes's theorem," "TAGI's framework" is wrong). For methods, concepts, or objects, use "of" instead: "the output of the network," not "the network's output."
...
- Library code lives in `triton_tagi/`. Training scripts and experiments live in `examples/`. Tests live in `tests/unit/`.
- One primary class per file; file name matches the class in lowercase (`linear.py` → `Linear`).
- No `print()` calls in library code; use `logging.getLogger(__name__)` if output is needed.
- No commented-out code in committed files — remove it entirely or move to a scratch branch.
- No bare `except:` clauses.

### 3.2 TAGI Tensor Naming

All tensor names follow the paper notation. Capital `S` always means variance (σ²), never standard deviation.

| Symbol | Meaning | Typical shape |
|---|---|---|
| `ma` | activation mean | `(N, ...)` |
| `Sa` | activation variance | `(N, ...)` |
| `mz` | pre-activation mean | `(N, ...)` |
| `Sz` | pre-activation variance | `(N, ...)` |
| `mw` | weight mean | `(in, out)` |
| `Sw` | weight variance | `(in, out)` or scalar tensor |
| `mb` | bias mean | `(1, out)` or `(out,)` |
| `Sb` | bias variance | `(1, out)` or `(out,)` or scalar tensor |
...
Shared-variance layers (`SharedVar*`) store `Sw`/`Sb` as scalar 0-d tensors rather than full matrices. The external interface (`layer.Sw`) is always a tensor; callers should not assume its shape.
...
- Private helpers: leading underscore (`_apply_constraints`)
- Loop counters that are intentionally unused: leading underscore (`_it`, `_oi`)

### 3.4 Type Annotations

All public functions and class methods have full type annotations. Use `from __future__ import annotations` at the top of every file to enable forward references without runtime cost. Do not annotate private helpers unless the types are non-obvious.
...
Every layer inherits from `Layer` or `LearnableLayer` (defined in `triton_tagi/base.py`).

- `Layer`: requires `forward` and `backward`. For activation layers (ReLU, Remax, Bernoulli, etc.) and structural layers (Flatten, AvgPool2D).
- `LearnableLayer(Layer)`: additionally requires `update(cap_factor)` and `num_parameters: int`. For layers with trainable parameters (Linear, Conv2D, BatchNorm2D, FRN2D, TLU, ResBlock variants).

`isinstance(layer, Layer)` and `isinstance(layer, LearnableLayer)` are the canonical checks in `Sequential`. Never use `hasattr(layer, "mw")` or similar duck-typing.

### 3.6 Docstrings

Google-style docstrings for all public classes and methods. Document the mathematical operation, not the implementation. Private methods do not need docstrings. Mathematical formulas belong in the docstring, not inline comments.
...
- `Sequential` moves all parameters to `device` during `__init__` via `_move_layer_to_device`.
- Never perform device placement inside `forward()` or `backward()`.

### 3.8 Invariants Every Layer Must Maintain

1. `Sa` is always non-negative after `forward`. Clamp with `Sa.clamp(min=0)` if numerical drift is possible.
2. `forward` is stateless for activation layers (no `self._cache`).
3. Learnable layers may cache intermediate tensors on `self` (e.g., `self.patches_ma`, `self.m_hat`). These are always overwritten at the start of each `forward` call.
4. All delta tensors and cached tensors reside on the same device as the layer's parameters.
5. `backward` must not be called before `forward` (the cache is populated by `forward`).
